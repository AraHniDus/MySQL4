...
CREATE DATABASE IF NOT EXISTS home_work4;
USE home_work4;
...
SELECT c.shops_id, c.name, s.id, s.shopname
FROM shops s 
JOIN cats c 
ON c.shops_id = s.id;
...
SELECT s.*, c_m.*
FROM shops s
JOIN (SELECT name, shops_id FROM cats WHERE name = "Murzik") AS c_m
ON s.id = c_m.shops_id;
...
AND DATE(orders.ord_datetime) BETWEEN '2020-02-05' AND DATE_ADD('2020-02-05', interval 1 WEEK);